...
namespace MyUILibrary
{
    ...
    public class RadialProgress : VisualElement
    {
        ...
        // DrawMeshes() uses the EllipseMesh utility class to generate an array of vertices and indices, for the
        // the progress ring . It then passes the geometry to the MeshWriteData
        ...
        void DrawMeshes(MeshGenerationContext context)
        {
            ...
            m_ProgressMesh.borderSize = 25;
            m_ProgressMesh.UpdateMesh();


            // Keep progress between 0 and 100
            float clampedProgress = Mathf.Clamp(m_Progress, 0.0f, MAX_AMOUNT);

            // Determine how many triangles are used to depending on progress, to achieve a partially filled circle
            int sliceSize = Mathf.FloorToInt((k_NumSteps * clampedProgress) /100f);
